...
bmap(struct inode *ip, uint bn)
{
  ...
  if (ip->type == T_EXTENT){   
    ...
    {
      if (fillBlock == -1 && ip->addrs[i] == 0)
	fillBlock = i;
      length = ip->addrs[i] & 0xFF;
      if (bn >= ip->addrs[i+1] && bn < length + ip->addrs[i+1]){
        addr = ip->addrs[i] >> 8;
	addr += bn - ip->addrs[i+1];
	return addr;
      }
      else if (bn == length + ip->addrs[i+1]){
        neighborBlock = i;
      }
    }  
    uint returnAddr;
    ...
    // check if newly allocated block is both logical neighbor and neighbor in 
    // block addr
    if (addr == (ip->addrs[neighborBlock] >> 8)+(ip->addrs[neighborBlock] & 0xFF) && neighborBlock >= 0){
	ip->addrs[neighborBlock] += 1; 
        return returnAddr;
    }
    // if block needs to be put on new place on disk
    if (fillBlock >= 0){
        addr <<= 8; // Leftshift 1-byte
        addr += 1;
	ip->addrs[fillBlock] = addr;
        ip->addrs[fillBlock + 1] = bn;
        return returnAddr; 
    }
  }    
  else {
...
itrunc(struct inode *ip)
{
  ...
  if (ip->type == T_EXTENT) {
    for (i = 0; i < NDIRECT; i+=2) {
      extentLength = ip->addrs[i] & 0xFF;
      for (j = 0; j < extentLength; j++) {
