...
{
  if (block > 13 || inode->direct[block] == -1) return -1;
...
int fsi_build(char *fsi_path){
  ...
  //write dir block
  int pos = 0;
  memcpy(segment, dir_block, 128*sizeof(dir_ent_t));
  ...
  memcpy((void *)(segment + pos), &root, sizeof(inode_t));
  inodes[0] = pos;
  ...
  memcpy(segment + pos, inodes, 16*sizeof(int));
  pos += 16*sizeof(int);
...
int fsi_lookup(int pinum, char* name)
{
  ...
  while(!done && block < NDIRECT){
    ...
    for(i = 0; i < 128; i++)
    {
      ...
      else if (strcmp(dir_block[i].name, name))
...
int fsi_creat(int pinum, int type, char *name)
{
  ...
  if(pnode.type != LFS_DIRECTORY) return -1;
  ...
  if(pnode.size < (foundBlock - 1) * LFS_BLOCK_SIZE + dirPos *sizeof(dir_ent_t))
    pnode.size = (foundBlock -1) *LFS_BLOCK_SIZE + dirPos * sizeof(dir_ent_t);
